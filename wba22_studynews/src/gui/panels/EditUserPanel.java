...
public class EditUserPanel extends JPanel {
	...
	public EditUserPanel(Application app, PubsubClient pubsub, UserDatabase user) {
		this.app = app;
		this.pubsub = pubsub;
		this.user = user;
		initUI();
	...
	protected void safeProfile() {
		user.getEintrag().get(0).setVorname(vornameField.getText());
		user.getEintrag().get(0).setNachname(nachnameField.getText());
		user.getEintrag().get(0).setStatus(statusField.getText());
		
		Client client = Client.create();
		
		try { 
			WebResource webResource = client
			   .resource("http://localhost:4434/user/"+user.getEintrag().get(0).getId().toString()+"/edit/");
	...
	private void loadProfile() throws JAXBException {
		...
		vornameField.setText(user.getEintrag().get(0).getVorname());
		nachnameField.setText(user.getEintrag().get(0).getNachname());
		statusField.setText(user.getEintrag().get(0).getStatus());
